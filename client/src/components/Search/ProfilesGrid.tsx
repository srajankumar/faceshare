      ...
      {!loading ? (
        <div className="w-full justify-center items-center max-w-7xl md:flex flex-wrap">
          {filteredSearchProfiles.map((profile) => (
            <Link
              key={profile._id}
              className="w-fit m-5 hover:scale-[102%] hover:rotate-1 transition-all duration-200"
              href={`https://faceshare.vercel.app/${profile.username}`}
            >
              <div className="flex border rounded-2xl h-40 md:w-80 flex-col justify-center items-center">
